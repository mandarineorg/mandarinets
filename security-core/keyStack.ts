...
// Mandarine.ts: This class has been obtained from https://github.com/oakserver/oak/blob/master/keyStack.ts
// Thanks Oak authors for this.
...
export class KeyStack {
  ...
  indexOf(data: Data, digest: string): number {
    for (let i = 0; i < this.#keys.length; i++) {
      if (SecurityUtils.compare(digest, this.#sign(data, this.#keys[i]))) {
        return i;
      }
    }
    return -1;
